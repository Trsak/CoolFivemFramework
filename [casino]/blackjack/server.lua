...
function getDeck()
    local tDeck = {}
    for _, rank in each(ranks) do
        for _, suit in each(suits) do
...
function GiveMoney(player, money)
    exports.inventory:addCasinoChips(player, math.tointeger(money))
end

function TakeMoney(player, money)
    exports.inventory:removeCasinoChipsByValue(player, math.tointeger(money))
end

function HaveAllPlayersBetted(table)
    for i, v in each(table) do
        if v.bet < 1 then
            return false
        end
    end
...
function CheckPlayerBet(i, bet)
    ...
    local playerChips = exports.inventory:getCasinoChipsTotalValue(client)
    ...
    TriggerClientEvent("BLACKJACK:BetReceived", client, canBet)
end
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if players[index] and #players[index] ~= 0 then
                ...
                repeat
                    for i, v in each(players[index]) do
                ...
                if #players[index] == 0 then
                    ...
                    if ArePlayersStillIn(players[index]) then
                        ...
                        for x = 1, 2 do
                            ...
                            for i, v in each(currentPlayers) do
                                if v.player_in then
                                    local card = takeCard(deck)
                                    ...
                                    if handValue(v.hand) == 21 then
                                        local winAmount = math.floor(v.bet * 2.5)
                                        TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "good", winAmount)
                                        DebugPrint("TABLE " .. index .. ": " .. GetPlayerName(v.player):upper() .. " HAS BLACKJACK")
                                        GiveMoney(v.player, winAmount)

                                        exports.logs:sendToDiscord(
                                            {
                                                channel = "casino",
                                                title = "Blackjack",
                                                description = "Hráč vsadil " .. exports.data:getFormattedCurrency(v.bet) .. " a má blackjack, vyhrál " .. exports.data:getFormattedCurrency(winAmount),
                                                color = "10181046"
                                            },
                                            v.player
                                        )
                            ...
                            if v.player_in == true and (handValue(v.hand) > handValue(dealerHand) or handValue(dealerHand) > 21) then
                                ...
                                if v.splitHand then
                                    if handValue(v.splitHand) > handValue(dealerHand) or handValue(dealerHand) > 21 then
                                        -- WIN
                                        TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "good", v.bet * 2)
                                ...
                                exports.logs:sendToDiscord(
                                    {
                                        channel = "casino",
                                        title = "Blackjack",
                                        description = "Hráč vsadil " .. exports.data:getFormattedCurrency(v.bet) .. " vyhrál " .. exports.data:getFormattedCurrency(v.bet * 2),
                                        color = "10181046"
                                    },
                                    v.player
                                )
                                ...
                                exports.logs:sendToDiscord(
                                    {
                                        channel = "casino",
                                        title = "Blackjack",
                                        description = "Hráč vsadil " .. exports.data:getFormattedCurrency(v.bet) .. " a výsledek je PUSH",
                                        color = "10181046"
                                    },
                                    v.player
                                )
                                ...
                                exports.logs:sendToDiscord(
                                    {
                                        channel = "casino",
                                        title = "Blackjack",
                                        description = "Hráč vsadil " .. exports.data:getFormattedCurrency(v.bet) .. " a prohrál",
                                        color = "10181046"
                                    },
                                    v.player
                                )
...
RegisterCommand("resetCasinoBlackJackPed", function(source, args)
    if exports.data:getUserVar(source, "admin") == 3 then
        TriggerClientEvent("casino:blackjack:clear", -1)
    end
end)