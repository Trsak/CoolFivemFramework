...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    if (OpenedChatPicture == null) {
        ...
        if (ChatData.profilepicture != null || ChatData.profilepicture != undefined || ChatData.profilepicture != "default") {
            OpenedChatPicture = ChatData.profilepicture
        }
...
QB.Phone.Functions.LoadWhatsappChats = function(chats) {
    ...
    $.each(chats, function(i, chat){
        ...
        if (chat.profilepicture !== "default" && chat.profilepicture !== undefined) {
        ...
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread).css({"display":"block"});
        } else {
...
QB.Phone.Functions.SendQuickGPS = function(cData) {
    $.post('https://phone/SendMessage', JSON.stringify({
        ChatNumber: cData.number,
        ChatDate: GetCurrentDateKey(),
        ChatMessage: "Shared location",
        ChatTime: FormatMessageTime(),
        ChatType: "location",
    ...
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                var Sender = "me";
                if (message.sender !== QB.Phone.Data.MetaData.simActive.primary.telNumber && message.sender !== QB.Phone.Data.MetaData.simActive.secondary.telNumber) { Sender = "other"; }
                var MessageElement
                if (message.type == "message") {
                    ...
                    MessageElement = '<div class="whatsapp-openedchat-message whatsapp-openedchat-message-'+Sender+' whatsapp-shared-location" data-x="'+message.data.x+'" data-y="'+message.data.y+'"><span style="font-size: 1.2vh;"><i class="fas fa-thumbtack" style="font-size: 1vh;"></i> GPS</span><div class="whatsapp-openedchat-message-time">'+message.time+'</div></div><div class="clearfix"></div>'
                }
