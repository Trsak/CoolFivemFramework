    ...
    function()
        ...
        while true do
            if not isDead and isSpawned then 
                ...
                local closestVehicle = getVehicleInDirection(6.0)
                if not DoesEntityExist(closestVehicle) then
                    closestVehicle = GetClosestVehicle(GetEntityCoords(ped), 6.0, 0, 70)
                end
            ...
            if not isDead and isSpawned then 
                ...
                if Vehicle.Vehicle ~= nil then
                    if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                        DrawText3D(Vehicle.Coords.x, Vehicle.Coords.y, Vehicle.Coords.z, "[SHIFT] + [E] Tlačení vozidla")
                    end
