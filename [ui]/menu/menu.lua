...
local gsFont = RegisterFontId("AMSANSL")
...
local titleFont = gsFont
local titleScale = 1.0

local buttonHeight = 0.038
local buttonFont = gsFont
...
local upKeyPress = 0
local downKeyPress = 0
local leftKeyPress = 0
local rightKeyPress = 0
local pressSpeed = 0
...
local defaultStyle = {
    ...
    titleBackgroundColor = { 109, 80, 160, 255 },
    titleBackgroundSprite = nil,
    subTitleColor = { 109, 80, 160, 255 },
...
local function drawTitle()
    ...
    if currentMenu.title then
        local titleLen = utf8.len(currentMenu.title)
        local actualScale = titleScale
        local actualTitleOffcet = titleYOffset

        if titleLen > 22 then
            actualScale = 0.4
            actualTitleOffcet = actualTitleOffcet + 0.013
        elseif titleLen > 20 then
            actualScale = 0.5
            actualTitleOffcet = actualTitleOffcet + 0.019
        elseif titleLen > 18 then
            actualScale = 0.6
            actualTitleOffcet = actualTitleOffcet + 0.016
        elseif titleLen > 16 then
            actualScale = 0.7
            actualTitleOffcet = actualTitleOffcet + 0.013
        elseif titleLen > 14 then
            actualScale = 0.8
            actualTitleOffcet = actualTitleOffcet + 0.01
        elseif titleLen > 12 then
            actualScale = 0.9
        end

        setTextParams(titleFont, getStyleProperty('titleColor'), actualScale, true)
        drawText(currentMenu.title, x, y - titleHeight / 2 + actualTitleOffcet)
    end
end
...
function WarMenu.CheckBox(text, checked, callback, customText)
    if not currentMenu then
        return
    end
    ...
    local pressed
    if customText then
        name = checked and customText[1] or customText[2]
        pressed = WarMenu.Button(text, name)
    else
        pressed = WarMenu.SpriteButton(text, 'commonmenu', name)
    end
    ...
    if currentMenu then
        ...
        if currentMenu.aboutToBeClosed then
            ...
            elseif IsDisabledControlPressed(0, keys.down) then
                downKeyPress = downKeyPress + 3

                if downKeyPress >= 50 - pressSpeed then
                    pressSpeed = pressSpeed + 20
                    downKeyPress = 0
            ...
            elseif IsDisabledControlPressed(0, keys.up) then
                upKeyPress = upKeyPress + 3

                if upKeyPress >= 50 - pressSpeed then
                    pressSpeed = pressSpeed + 20
                    upKeyPress = 0
            ...
            elseif IsDisabledControlPressed(0, keys.left) then
                ...
                if leftKeyPress >= 50 - pressSpeed then
                    pressSpeed = pressSpeed + 3
                    leftKeyPress = 0
                    currentKey = keys.left
                end
            elseif IsDisabledControlPressed(0, keys.right) then
                ...
                if rightKeyPress >= 50 - pressSpeed then
                    pressSpeed = pressSpeed + 3
                    rightKeyPress = 0
                    currentKey = keys.right
                end
...
function WarMenu.SetCurrentOption(option)
    currentMenu.currentOption = option
end
