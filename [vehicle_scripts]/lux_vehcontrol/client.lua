...
CONTROLS	
	...
	Number 4 Key: Scene Light Toggle
---------------------------------------------------
]]
local TARight = 157
local TALeft = 160
local TACenter = 158
local SceneLights = 164
...
function playClick(soundFile, soundVolume)
    SendNUIMessage(
        {
    ...
    function()
        while true do
            ...
            if IsPedInAnyVehicle(playerped, false) then
                ...
                local vehnamebruh = GetDisplayNameFromVehicleModel(GetEntityModel(veh))
                local checkeroni = GetTextSubstring(vehnamebruh, 2, 4)
                    ...
                    if GetVehicleClass(veh) == 18 then
                        ...
                        if not IsPauseMenuActive() then
                            ...
                            elseif IsDisabledControlJustReleased(0, 19) or IsDisabledControlJustReleased(0, 82) then
                                ...
                                if cstate == 0 then
                                    if IsVehicleSirenOn(veh) then
                                        playClick("Upgrade", 0.7) -- Upgrade
                                        SetLxSirenStateForVeh(veh, 1)
                                        shouldSend = true
                                    end
                                else
                                    playClick("Downgrade", 1) -- Downgrade
                                    SetLxSirenStateForVeh(veh, 0)
                                    shouldSend = true
                                end
                            end
