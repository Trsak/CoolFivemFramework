	...
	function()
		...
		if nativeAudio == "false" and _3dAudio == "false" and _2dAudio == "false" then
			if gameVersion == "fivem" then
				...
				logger.info(
					"No convars detected for voice mode, defaulting to 'setr voice_use3dAudio true' and 'setr voice_useSendingRangeOnly true'"
				)
			end
		...
		if GetConvar("gamename", "fivem") == "rdr3" then
			if nativeAudio == "true" then
				logger.warn(
					"RedM doesn't currently support native audio, automatically switching to 3d audio. This also means that submixes will not work."
				)
...
RegisterCommand(
	...
	function(source, args)
		...
		if exports.data:getUserVar(_source, "admin") > 1 then
			if tonumber(args[1]) and GetPlayerName(tonumber(_source)) then
			else
				TriggerClientEvent(
					"chat:addMessage",
					_source,
